...
export function createRingBuffer(m: Mem, opt?:{
    ...
    const r = {
        read: new Uint32Array(m.allocLines(1)),
        write: new Uint32Array( m.allocLines(capacity+1)),
        capacity,
        entrySize
    }
    return r
...
export function ringWrite(r: RingBuffer, e: Uint32Array)  {
        while (true) {
            ...
            if (next!=read){
                ...
                r.write.slice(firstDataSlot+write,firstDataSlot+write+r.entrySize).set(e)
